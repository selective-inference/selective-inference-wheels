#################################################
Building and uploading selective inference wheels
#################################################
...
https://travis-ci.org/selective-inference/selective-inference-wheels

Appveyor interface at
https://ci.appveyor.com/project/jonathan-taylor/selective-inference-wheels

The driving github repository is
https://github.com/selective-inference/selective-inference-wheels
...
* builds a selective-inference wheel, linking against these fresh builds;
...
* making a commit to the `selective-inference-wheels` repository (e.g. with `git
...
Which selective-inference commit does the repository build?
===========================================================

The ``selective-inference-wheels`` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -u $CDN_URL -s -v -w ~/wheelhouse -t macosx selective-inference $VERSION
    wheel-uploader -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 selective-inference $VERSION
    wheel-uploader -u $CDN_URL -s -v -w ~/wheelhouse -t win selective-inference $VERSION
...
``selective-inference`` is the root name of the wheel(s) to download / upload, and
...
``selective-inference-0.1.0-`` from http://wheels.scipy.org to ``~/wheelhouse``, then
upload them to PyPI.
